...
## Sep 22 ~4:05 PM ET — THE YARDSTICK (`5fcae03a` data, `4748ece3` app; founder brief of 3 PM)
His ask: "bring back the yardstick... for darts... for prop picks... an interactive table where I can move the yardstick part... tabs I can click on", with his HOF screenshots (Leaders: stat tabs, "At Least" ruler, window tabs, ranked hit-rate table; Player: window rates, bars against a dashed line).
- **Data:** every MLB player card now stores `payload.log`: the last 20 regular-season finals (date, opponent, one array per stat) and `ge[stat][k-1]` = season games with at least k. Stats: hitters h, tb, hr, r, rbi, hrr, sb, k, bb; pitchers k, outs, er, ha, bb (same readers as the prop-rate code). Runs with the daily card build; today's 297 cards rebuilt by hand. Adds ~1.2 KB per card.
- **One component set** (`ios/GaryApp/Darts/GaryYardstick.swift`): `GaryRuler` (brass tick tape under a fixed gold needle; drag, fling, tap; a click per mark; horizontal pans hold the page and any carousel still, vertical ones fail at once so the page scrolls), `LogWindowTabs` (L5 / L10 / L20 / SEASON as text tabs, each with its own "7 of 10"), `LogBarsChart` (green reached, red not, dashed gold line at the posted-line equivalent with its number; SEASON draws the spread of games at each count), `PlayerLogPanel` (all of it). Counts only, never percentages; green at 60%+, red under 35%.
- **Darts → HIT RATES** (`DartsHitRates.swift`, between TODAY'S DARTS and STREAKS): stat tabs (hits, total bases, H+R+RBI, home runs, runs, RBI, stolen bases, strikeouts, outs), the ruler, "2+ HITS" beside the window tabs, then tonight's players ranked by the window's count (ties by the season at the same mark) with their last games as small bars. A row opens the player card on the same stat, mark and window.
- **Player card:** "Hit rates" after The read, posted lines first; the price shows when the ruler sits on a posted line; the old "The angle" prop list steps aside when the panel shows (NFL cards keep it).
- **Winners prop breakdown:** the prop's stat under the line tracker, opened at the line, over or under ("2 OR FEWER EARNED RUNS"). The breakdown now loads the play's own day of cards (was today only).
- **Headers:** the shared header sits on the safe area (top padding 10 → 0, rule spacing 10 → 4); Darts tightened its rows; Winners' TODAY head drops the date the header says.
- **Whole words:** breakdown names, lines, books, prop tickets, the live card's back and parlay legs wrap instead of shrinking into "…". Slip tab slimmed into the margin (24pt) and its count is a plain numeral (it was a filled circle).
- **NFL not ported yet** (the reuse law wants the reason): NFL cards store a 5-game log (Week 3: mostly last season), and yards need a ruler stepping by 5 or 10. The port needs a two-season log in `footballPlayerInsightCards.js` (one more BDL batch per game) and a stepped ruler. ⚑ Next pass.
- **Tour:** posts reach the simulator only through `xcrun simctl spawn <UDID> notifyutil -p com.gary.tour`; host `notifyutil` seemed to work for `tab N` only because the tab persists. `scroll` now skips opacity-parked pages; new verbs `darts stat|mark|window|rate`.
