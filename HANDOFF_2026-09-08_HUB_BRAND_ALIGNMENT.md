...
Unused version 2.25/build 914 also integrates the launch review's independently owned fixes:
...
- Upload succeeded September 8 at **13:33:26 UTC** (9:33:26 AM Eastern), with `Upload succeeded`, `Uploaded GaryApp`, `EXPORT SUCCEEDED` and terminal exit 0. Apple reports the package processing. Internal TestFlight availability remains unverified at this point. **Do not upload 914 again.**
...
The launch root owns Apple processing/internal TestFlight confirmation and App Review selection. A temporary shared push hold applies while its separate stored Hub correction is verified; this session's final release docs must not be pushed until the root clears that hold. The native upload is unaffected. Other active launch/backend work and private local configuration remain outside this session's commits.
