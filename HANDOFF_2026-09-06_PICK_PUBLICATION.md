...
## Released and verified — 10:19 ET

- Source `3173b93c9bb0ed7496a244e2858372610a14ea89` is pushed to main. GitHub Verify `34038631426` passed that exact source, including its integration checks.
- Vercel production `dpl_J8cvTfXC9Y4FPkHxL2PG5fZZ98Ts` is READY at the same SHA with both public domains and their www aliases assigned. A fresh browser displays September 6's 18-game board with zero premature publications and the first expected MLB window at 10:40 AM. The September 5 archive displays all 45 original game tickets, including the unchanged East Carolina +28.5 selection.
- After confirming the scheduler had no active child jobs, one controlled reload at 10:17:19 ET started owner PID 86368 / Node PID 86369. Its startup identifies source `3173b93c`, game era `c1e0e5919df1`, props era `aa5fa0ab453b`, and the canonical backend directory. Receipt: `/tmp/gary-publication-scheduler-reload-20260906.json`.
- The restarted process successfully published the slate at 10:17:25, today's board at 10:17:59, and tomorrow's board at 10:18:27. It armed the ordinary trigger queues and retained Brewers–Reds at 10:40 AM as the next batch. No active pick was interrupted and no test pick was published.
- Final health at 10:19:05 ET verified 18/18 board games, all current MLB/college card coverage, and 45/45 previous-day grades/recaps. Log: `/tmp/gary-publication-health-final-20260906.log`.
- `production-truth.js` verified the new canonical scheduler, unchanged Astra-game/Sol-prop routing, Winners PID 11032, all 20 edge deployment timestamps, and no unpushed code. Its exit 1 is solely the intentionally uncommitted real Firebase configuration. Log: `/tmp/gary-publication-production-20260906.log`.

This proves the repair is deployed, the failure paths are tested, and current reads are healthy. The first newly generated ticket under this release and the next overnight cycle have not yet occurred. Existing original tickets were not regenerated or replaced. The earlier protected obsolete-cron removal is outside this repair and must not be reported as completed by the scheduler reload.
