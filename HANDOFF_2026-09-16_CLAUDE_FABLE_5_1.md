...
**Latest September 16 root-data repair:** Read [the integration audit](HANDOFF_2026-09-16_PICK_DATA_ROOT_CAUSES.md). Adam explicitly requested repairing the causes beyond validation. MLB team aliases, recent-box queries/identity joins, June's Savant input contract and actual pitcher-start metadata are repaired; college-football profiles now reach the dedicated standings reader. The real report rebuild and 4,206 backend tests pass. June/game stamp **486898ab19cc**; new cache namespace prevents reuse of older reports. The narrow report/orchestrator data changes are recorded exceptions; judgment/prompts and the 300-pick cohort remain intact. NFL opening-week gaps and the incomplete local CLI parity check are documented without claiming universal health.
