...
export async function GET(req: Request) {
  ...
  const [displayFont, uiFont, interRg, interSb, bear] = await Promise.all([
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: CARD, border: `1px solid rgba(201,162,39,0.34)`, borderRadius: 28, padding: '56px 60px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ fontFamily: 'Gary UI', fontSize: 28, color: GOLD, letterSpacing: 4 }}>{"GARY'S TAKE"}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ fontFamily: 'Gary UI', fontSize: 20, color: 'rgba(255,255,255,0.42)', letterSpacing: 1, marginRight: 20 }}>{matchup}</div>
          ...
          {/* the pick(s), in gold */}
          <div style={{ fontFamily: 'GaryDisplay', fontSize: 64, color: GOLD, marginTop: 26, lineHeight: 1 }}>{picks}</div>
