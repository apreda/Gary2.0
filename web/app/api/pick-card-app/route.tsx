...
// gold "GARY'S PICK" eyebrow + bear, the pick as big stacked GaryDisplay display type, a sport-accent league
...
export async function GET(req: Request) {
  ...
  const [displayFont, uiFont, interRg, interMd, bear] = await Promise.all([
      ...
      <div style={{ width: '100%', height: '100%', display: 'flex', background: '#000' }}>
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: CARD, border: '1px solid rgba(255,255,255,0.10)', borderRadius: 28, padding: '60px 64px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div style={{ fontFamily: 'Gary UI', fontSize: 30, color: GOLD, letterSpacing: 6 }}>{eyebrow}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: 14 }}>
            <div style={{ fontFamily: 'GaryDisplay', fontSize: heroSize, color: WHITE, lineHeight: 0.96 }}>{hero1}</div>
            <div style={{ fontFamily: 'GaryDisplay', fontSize: heroSize, color: WHITE, lineHeight: 0.96 }}>{hero2}</div>
          </div>

          {/* meta line: teal token + opponent + gold odds */}
          <div style={{ display: 'flex', alignItems: 'center', marginTop: 28 }}>
            <div style={{ fontFamily: 'Gary UI', fontSize: 26, color: TEAL, letterSpacing: 2 }}>{token}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontFamily: 'Gary UI', fontSize: 24, color: GOLD, letterSpacing: 1 }}>{time}</div>
            <div style={{ fontFamily: 'Gary UI', fontSize: 24, color: 'rgba(201,162,39,0.75)', letterSpacing: 2 }}>{"GARY'S TAKE  ›"}</div>
          </div>
        </div>
      </div>
    ),
