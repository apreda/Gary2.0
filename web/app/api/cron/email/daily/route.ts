...
import { runDailyBoardCampaign } from '@/lib/email/send';
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await runDailyBoardCampaign();
    ...
    console.error(JSON.stringify({ event: 'email_campaign_failed', kind: 'daily_board', code: error instanceof Error ? error.name : 'unknown' }));
