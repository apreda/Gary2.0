      ...
      <nav className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <details className="relative md:hidden">
            <summary className="cursor-pointer list-none rounded-lg border border-white/15 px-3 py-2 text-xs text-white/70">
              Menu
            </summary>
            <div className="absolute right-0 top-full mt-2 w-48 rounded-xl border border-white/10 bg-card shadow-[0_8px_24px_rgba(0,0,0,0.6)]">
              {LINKS.map(l => (
                <Link
                  key={l.href}
                  href={l.href}
                  className="block px-4 py-3 text-sm text-white/70 transition-colors hover:text-white/95 first:rounded-t-xl last:rounded-b-xl"
                >
                  {l.label}
                </Link>
              ))}
