...
// ── THE OVERNIGHT LINE ──────────────────────────────────────────────────────
// The graded record as ONE quiet row (founder, Jul 27: the big strip crowded
// the door — headline stories and UP NEXT lead the page now). Still the
// Winners funnel: the whole line is the tap.
struct HomeOvernightLine: View {
    let record: (w: Int, l: Int, p: Int)
    let net: Double?
    let label: String
    var roll: (line: String, odds: String)? = nil
    let onOpen: () -> Void

    private var netText: String? {
        guard let net else { return nil }
        let d = Int((abs(net) * 100).rounded())
        return net >= 0 ? "+$\(d)" : "-$\(d)"
    }
        ...
        Button(action: onOpen) {
            HStack(spacing: 8) {
                ...
                if let netText {
                    Text(netText)
                        .font(GaryFonts.mono(12, bold: true))
                        .foregroundStyle((net ?? 0) >= 0 ? GaryColors.win : GaryColors.loss)
                }
                Spacer(minLength: 8)
                if let roll {
                    Text("✓ \(roll.line) \(roll.odds)")
                        .font(GaryFonts.mono(10.5, bold: true))
                        .foregroundStyle(GaryColors.gold)
                        .lineLimit(1).minimumScaleFactor(0.8)
                }
