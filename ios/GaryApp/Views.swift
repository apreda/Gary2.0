...
struct HomeView: View {
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    // Three orderings of the same blocks — results in the
                    // morning, the slate pre-game, the tape + takeover when
                    // games are live. (World Cup module parked — it used to lead
                    // the page; restore on request.)
        ...
        .task {
            ...
            do {
                try await withTimeout(seconds: 30) {
                    ...
                    async let pulseFetch = SupabaseAPI.fetchMarketPulse(date: SupabaseAPI.hubGradedDateEST())
    ...
    @ViewBuilder private var liveSections: some View {
        ...
        if !liveSlateRows.isEmpty {
            HomeSlateSection(header: "Tonight's board", sub: liveBoardSub, rows: liveSlateRows) {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { selectedTab = 1 }
            }
            .opacity(animateIn ? 1 : 0)
            .animation(.easeOut(duration: 0.6).delay(0.12), value: animateIn)
    ...
    /// The home's state switcher — underline tabs (the Winners "Terminal Tape"
    ...
    private var liveBoardSub: String {
        let lives = liveScoresNow.filter { $0.isLive }.count
        let finals = liveScoresNow.filter { $0.isFinal && pickFor($0) != nil }.count
        let upcoming = max(0, todayPicks.count - lives - finals)
        return "\(finals) settled · \(lives) live · \(upcoming) upcoming"
    }

    /// Tonight so far, from finals Gary had a side in.
    private var tonightRecord: (w: Int, l: Int) {
        var w = 0, l = 0
        for ls in liveScoresNow where ls.isFinal {
            guard let p = pickFor(ls) else { continue }
            switch HomeLiveVerdict.evaluate(pick: p, live: ls) {
            case .covering: w += 1
            case .trailing: l += 1
            case .neutral: break
            }
    ...
    private var liveCountText: String? {
        let n = liveScoresNow.filter { $0.isLive }.count
...
/// ③b Hits & heartbreakers — last night in two beats: the biggest cash and the
/// worst beat (the favorite that didn't hold), side by side. The honest recap a
...
struct HomeHitsSection: View {
    ...
    private let won = Color(hex: "#3FB950")
    private let lost = Color(hex: "#E5484D")
...
/// League-wide market scoreboard for the settled night — overs, ML
...
/// The Wire — around-the-league items written from the bettor's seat:
/// results vs the closing number, line moves, injuries with the market
/// consequence, curated X voices (quoted behind a hairline rule).
struct HomeWireSection: View {
    let items: [SupabaseAPI.WireItem]
    let sub: String
    var pulse: [SupabaseAPI.MarketPulseRow] = []
    var limit: Int = 4
    /// The one open row — tap expands IN PLACE (no sheet, no overlay).
    ...
    @ViewBuilder private func wireRow(_ item: SupabaseAPI.WireItem) -> some View {
        let isVoice = item.kind == "voice"
        let rid = item.id ?? -1
        let expanded = expandedId == rid
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 7) {
                if isVoice {
                    Text("𝕏")
                ...
                Button { plansFocus = nil; showPlansSheet = true } label: {
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text("Every sport's Winners board · the plays Gary backs")
                                .font(.system(size: 11))
                                .foregroundStyle(.white.opacity(0.45))
                        }
                        Spacer(minLength: 8)
...
struct PlansSheetView: View {
    ...
    @ViewBuilder private var recordBlock: some View {
        if let s = ledger {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("Every pick Gary has made, graded against final scores the next morning. No deletions, no restatements — losses stay on the books with the wins.")
                    .font(.system(size: 13))
                    .foregroundStyle(.white.opacity(0.6))
                    .lineSpacing(3)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var allPlansScreen: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                HubSectionHeader(eyebrow: "All-Access", sub: "Every board, one plan")
    ...
    private var sportGridSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HubSectionHeader(eyebrow: "Single sports", sub: "One Winners board, \(GaryPricing.single) a month")
    ...
    private var bundleSection: some View {
        let ready = bundlePick.count >= 2
        let bundleSelected = selection == .bundle && ready
        return VStack(alignment: .leading, spacing: 10) {
            HubSectionHeader(eyebrow: "Bundles", sub: "Two sports \(GaryPricing.twoSport)/mo · three \(GaryPricing.threeSport)/mo")
            VStack(alignment: .leading, spacing: 0) {
                HStack(spacing: 0) {
                    ForEach(Self.sports, id: \.self) { lg in
                        let on = bundlePick.contains(lg)
                        Button { toggleBundle(lg) } label: {
                            Text(lg)
                                .font(GaryFonts.mono(11.5, bold: on)).tracking(0.4)
                                .foregroundStyle(on ? GaryColors.gold : .white.opacity(0.4))
                                .frame(maxWidth: .infinity, minHeight: 46)
                                .contentShape(Rectangle())
                        }
                        .buttonStyle(.plain)
                        .accessibilityLabel("\(lg)" + (on ? ", in bundle" : ""))
                        .accessibilityAddTraits(on ? [.isButton, .isSelected] : .isButton)
                    }
                }
                .padding(.horizontal, 6)
                Rectangle().fill(Color.white.opacity(0.06)).frame(height: 1)
                Button { if ready { select(.bundle, keepBundle: true) } } label: {
                    HStack(spacing: 12) {
                        Image(systemName: "square.grid.2x2")
                            .font(.system(size: 13, weight: .semibold))
                            .foregroundStyle(bundleSelected ? GaryColors.gold : .white.opacity(0.45))
                            .frame(width: 20)
                        VStack(alignment: .leading, spacing: 2) {
                            Text("BUNDLE")
                                .font(GaryFonts.mono(12, bold: true)).tracking(0.8)
                                .foregroundStyle(.white.opacity(0.9))
                            Text(ready ? bundlePick.sorted().joined(separator: " · ")
                                 : "Pick two or three sports above")
                                .font(.system(size: 11))
                                .foregroundStyle(.white.opacity(0.45))
                        }
                        Spacer(minLength: 8)
                        if ready {
                            Text(bundlePick.count == 3 ? GaryPricing.threeSport : GaryPricing.twoSport)
                                .font(GaryFonts.mono(13, bold: true))
                                .foregroundStyle(GaryColors.gold)
                        }
                        miniMark(bundleSelected)
                    }
                    .padding(.vertical, 12).padding(.horizontal, 14)
                    .contentShape(Rectangle())
                }
            ...
            .background(cardBackground(selected: bundleSelected))
            .padding(.horizontal, 16)
        }
    }
    ...
    private var includedFreeSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HubSectionHeader(eyebrow: "Free, always", sub: "The research is the resource — open to everyone")
            VStack(spacing: 0) {
    ...
    private var ctaEnabled: Bool {
        if case .bundle = selection { return bundlePick.count >= 2 }
        return true
    }
    ...
    private var ctaLabel: String {
        ...
        case .bundle:
            if bundlePick.count < 2 { return "PICK TWO OR THREE SPORTS" }
            return bundlePick.count == 3 ? "START THREE-SPORT BUNDLE — \(GaryPricing.threeSport)/MO"
                                         : "START TWO-SPORT BUNDLE — \(GaryPricing.twoSport)/MO"
        }
    }
    ...
    private var ctaCaption: String {
        ...
        case .single(let lg):
            return "Every \(lg) play Gary backs — the Winners board. \(GaryPricing.single)/mo, cancel anytime. \(tail)"
        case .bundle:
            if bundlePick.count < 2 {
                return "Any two sports \(GaryPricing.twoSport)/mo · any three \(GaryPricing.threeSport)/mo. Cancel anytime."
            }
            let p = bundlePick.count == 3 ? GaryPricing.threeSport : GaryPricing.twoSport
            return "\(bundlePick.sorted().joined(separator: " · ")) — \(p)/mo. Cancel anytime. \(tail)"
        }
    }
    ...
    private func primaryAction() {
        ...
        case .bundle:
            guard bundlePick.count >= 2 else { return }
            onBundle(Array(bundlePick))
        }
    }

    // MARK: Selection helpers

    private func select(_ s: PlanSelection, keepBundle: Bool = false) {
    ...
    private func toggleBundle(_ lg: String) {
        withAnimation(reduceMotion ? nil : .easeInOut(duration: 0.15)) {
            if bundlePick.contains(lg) { bundlePick.remove(lg) }
            else if bundlePick.count < 3 { bundlePick.insert(lg) }
            selection = bundlePick.count >= 2 ? .bundle : .allAccess
        }
    }
    ...
    /// A compact selectable single-sport tile for the all-plans grid.
    private func sportCard(_ lg: String) -> some View {
        let on = selection == .single(lg)
        let color = lg == "MLB" ? GaryColors.mlbGrass : Sport.from(league: lg).accentColor
        return Button { select(.single(lg)) } label: {
            VStack(alignment: .leading, spacing: 7) {
                HStack {
                    HStack(spacing: 6) {
                ...
                Text("Winners board").font(.system(size: 11.5)).foregroundStyle(.white.opacity(0.45))
                HStack(alignment: .firstTextBaseline, spacing: 2) {
                    Text(GaryPricing.single).font(GaryFonts.mono(13, bold: true)).foregroundStyle(GaryColors.gold)
                    Text("/mo").font(GaryFonts.mono(9)).foregroundStyle(.white.opacity(0.35))
                }
            }
            .padding(13)
            .frame(maxWidth: .infinity, alignment: .leading)
        ...
        .buttonStyle(.plain)
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("\(lg) pass. \(GaryPricing.single) a month. The Winners board." + (on ? " Selected." : ""))
        .accessibilityAddTraits(on ? [.isButton, .isSelected] : .isButton)
    }
    ...
    private func infoRow(icon: String, title: String, sub: String, tag: String) -> some View {
        HStack(spacing: 12) {
            Image(systemName: icon)
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(.white.opacity(0.5)).frame(width: 20)
            VStack(alignment: .leading, spacing: 2) {
            ...
            Spacer(minLength: 8)
            Text(tag).font(GaryFonts.mono(10, bold: true)).tracking(1).foregroundStyle(.white.opacity(0.4))
        }
        .padding(.vertical, 12).padding(.horizontal, 14)
        .accessibilityElement(children: .combine)
    }
    ...
    private func loadRecord() async {
        ...
        async let gamesTask = try? SupabaseAPI.fetchAllGameResults(since: nil)
        async let propsTask = try? SupabaseAPI.fetchPropResults(since: nil)
        let games = (await gamesTask) ?? []
        let props = (await propsTask) ?? []
        ...
        for pr in props { if let m = norm(pr.result), let d = pr.game_date { marks.append((d, m)) } }
        ...
        await MainActor.run {
            withAnimation(reduceMotion ? nil : .easeOut(duration: 0.3)) { ledger = stats }
        }
    }
}
...
func teamChipStyle(team: String, league: String?) -> (color: Color, label: String) {
    ...
    let label = lg == "WC" ? abbr : (abbr.count <= 2 ? abbr : String(abbr.prefix(1)))
    return (color, label)
}
...
struct TeamColorChip: View {
    ...
    var body: some View {
        ...
        let fontScale: CGFloat = style.label.count >= 3 ? 0.27 : (style.label.count == 2 ? 0.33 : 0.44)
        ZStack {
...
private func sharePreviewPick(league: String, away: String, home: String,
...
/// lands here; PropsHubView consumes it whenever the tab becomes visible
...
struct PropsHubView: View {
    ...
    /// (opacity-switched, never recreated), so onAppear/.task fire once per
    /// process — visibility flips drive the staleness refetch and deep-link
    /// consumption instead.
    ...
    /// When the page last loaded (30-min staleness gate) and for which EST
    /// date (3am-rollover gate) — checked on tab visibility and foreground.
    @State private var loadedAt: Date? = nil
    @State private var loadedDate: String = ""
    /// A fetch errored AND no edges rendered — show the retry surface, not
    /// the misleading "posts with lineups" copy.
    ...
    /// Last night league-wide (night_highlights) — the Night Board.
    ...
    /// Fetch today's connections for all leagues so the in-app league toggle
    /// works without a refetch. Maps Connection -> Signal, drops unmappable
    /// rows. Safe to re-run (pull-to-refresh / staleness gate) — the user's
    ...
    private func load() async {
        ...
        if dropped > 0 {
            // A backend lane rename would otherwise vanish silently.
            print("[PropsHubView] dropped \(dropped) row(s) with unmappable league/category — check SignalKind.from against the pipeline")
        }
    ...
    /// Re-fetch when the data is from a previous EST date (3am rollover),
    /// older than 30 minutes, or the last attempt failed.
    private func reloadIfStale() async {
        guard didLoad else { return }
        let expired = loadedAt.map { Date().timeIntervalSince($0) > 1800 } ?? true
        if loadedDate != SupabaseAPI.todayEST() || expired || fetchErrored {
            await load()
        }
    }
    ...
    private func consumeFocus() {
        ...
        case .firstInning, .runningGame, .parkWeather:
            condTab = lane
            pendingScrollAnchor = "conditions"
    ...
    var body: some View {
        ...
        // Kept-alive tabs never re-run .task — visibility flips and app
        // foregrounding drive deep-link consumption + the staleness refetch.
    ...
    // ---- Player-edge lane tabs (platoon / heat / ballpark / cooling / starters) ----
    ...
    // ---- Conditions lane tabs (first inning / running game / park & weather) ----

    /// The game-condition lanes, filtered to lanes that have rows tonight.
    private var conditionLanes: [SignalKind] {
        [SignalKind.firstInning, .runningGame, .parkWeather].filter { !items($0).isEmpty }
    }
    private var activeCondLane: SignalKind {
        if let t = condTab, conditionLanes.contains(t) { return t }
        return conditionLanes.first ?? .firstInning
    }
    private func condTitle(_ k: SignalKind) -> String {
        switch k {
        case .firstInning: return "FIRST INNING"
        case .runningGame: return "RUN GAME"
        case .parkWeather: return "PARK & WEATHER"
        default: return k.chip
        }
    }
    private func condSub(_ k: SignalKind) -> String {
        switch k {
        case .firstInning: return "NRFI & YRFI reads on tonight's slate"
    ...
    private var searchBar: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 11, style: .continuous)
                ...
                .overlay(RoundedRectangle(cornerRadius: 11, style: .continuous).stroke(GaryColors.warmWhite.opacity(0.07), lineWidth: 1))
        )
    }

    /// One search for everything the page shows: tonight's edges (every
    /// league), the graded receipts, live streaks, and last night's board.
    private var searchResults: some View {
        ...
        let edgeMatches = source.filter(hits)
        let receiptMatches = ydaySignals.filter(hits)
        let streakMatches = streakRows.filter {
            ($0.subject ?? "").lowercased().contains(q)
                || ($0.team ?? "").lowercased().contains(q)
                || ($0.detail ?? "").lowercased().contains(q)
        }
        let nightMatches = nightRows.filter {
    ...
    /// Compact read-only row for streak/night-board search matches.
    private func searchAuxRow(title: String, sub: String, trail: String, divider: Bool) -> some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
    ...
    /// First-load spinner — without it the page flashes a false "no edges
    /// yet" while the fetch is still in flight.
    private var hubLoadingState: some View {
        VStack(spacing: 12) {
            ProgressView().tint(GaryColors.gold)
    ...
    /// Fetch failed AND nothing rendered — honest copy, distinct from the
    /// pre-lineup quiet morning, with a retry path.
    private var hubErrorState: some View {
        VStack(spacing: 10) {
