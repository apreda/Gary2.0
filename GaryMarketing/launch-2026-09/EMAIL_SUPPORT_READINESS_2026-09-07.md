...
This removes the presumption that there is a known current stored audience awaiting the September 9 message. It does not prove that no historical/deleted or externally managed audience ever existed, nor does it verify sender-domain ownership, delivery, unsubscribe behavior or the full storage contract. Exact mailing address/test-recipient inputs and provider/storage acceptance remain required despite the empty tables. Do not populate the list merely to mark readiness complete.
