...
| 2. MLB research and restart | **Sonnet 5 subscription → Codex GPT-5.6 Luna subscription → wait**. Full successful messages and tool responses survive a switch. No API researcher fallback. Claude account usage credits were switched off; automatic reload was already off. The MLB scheduler hold was removed and the plist reloaded. Metered search is explicitly capped at zero. The game brain remains Sol. |
...
| 4. Winners admission | New policy and both migrations are live; the independent worker was restarted on the new code. Original evidence is compared across the actual daily start windows. Details below. |
...
- A supported **clear** pick or **lean** can cover a window. Additional places
  require clear evidence. **Toss-ups** and **unsupported** picks cannot enter.
  No confidence score, payout or automatic underdog rule is used.
- A sixth is allowed only in the final window, when the normal target is five,
  ...
  hard limit per sport for game picks. Props retain their separate policy.
...
- Claims open at T−90. Allow an incomplete early batch to arrive until T−65,
  ...
  publish stale or duplicate tickets. Completed judgments are not resampled
  until something qualifies. Missing evidence never manufactures a pick.

At verification, MLB had 15 games → target 4 across four windows; NCAAF had
45 games → target 5 across four windows. The old rule had already admitted
three noon NCAAF tickets. Those immutable published tickets consume today's
places, so today's distribution cannot meet every new reservation within five.
Future slates begin entirely under the new rule.
...
unavailable, cancellation and brain routing. They verify transport behavior,
...
The restored cohort still has **10 of 300 required MLB picks** at this checkpoint:
four stamped `80c55f8305ca`, six stamped `dc4bc0060d88`, alongside four older-system
September 11 picks. Today's permitted model/plumbing changes produce full surface
...
all 15 MLB games were pending and none had started without a pick. No open profile
...
- Backend: **374 files / 4,086 tests passed**. The later audit-only adjustment
  also passed its **18-test** production-truth suite.
...
- Winners isolated PostgreSQL: **23 tests passed**, including both migrations,
  permissions, reservations, sixth admission, concurrency and immutability.
