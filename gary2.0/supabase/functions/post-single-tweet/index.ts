import { isSocialServiceRequest } from "./authorization.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const authHeader = await generateOAuthHeader("POST", url, {}, apiKey, apiSecret, accessToken, accessTokenSecret);
    ...
    if (!response.ok) return new Response(JSON.stringify({ error: "Failed", details: data }), { status: 500, headers: { "Content-Type": "application/json" } });
    return new Response(JSON.stringify({ success: true, tweetId: data?.data?.id }), { headers: { "Content-Type": "application/json" } });
  } catch (err) {
