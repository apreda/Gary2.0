// Gary's persona prompt — Deno-side copy for the Edge Function.
// Source of truth: src/services/garyChat/persona.js
...
export function formatPicksMemoryBlock(picks: any[], contexts: any[]): string {
  ...
  const ctxByPickId = new Map<string, any>();
  ...
  for (const p of picks) {
    ...
    if (p.rationale) {
      ...
      lines.push(String(p.rationale).slice(0, 2500));
    }
    if (ctx.bilateral_case) {
      lines.push(`\n**Your bilateral case (what you wrote before deciding):**`);
      lines.push(String(ctx.bilateral_case).slice(0, 3000));
    }
    if (ctx.research_briefing) {
      lines.push(`\n**Research briefing your assistant pulled:**`);
      lines.push(String(ctx.research_briefing).slice(0, 3500));
    }
  }
