...
function fake(options: { rows?: any[]; stripeFailure?: boolean; mismatch?: boolean; authFailure?: boolean; deleteFailure?: boolean } = {}) {
  ...
  const request = ((input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.endsWith("/auth/v1/user")) return Promise.resolve(options.authFailure ? response({}, 401) : response({ id: owner }));
