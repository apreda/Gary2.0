-- GARY'S STREAK STARTS ON SEP 24 (founder, Sep 24 2026: "let's wipe that,
-- and that will just start with today's pick"). The streak, its best and
-- its last five count only streak picks from Sep 24 on; earlier days are no
-- streak day at all, so yesterday's pick carries no streak mark before then.
-- The earlier streak_picks rows stay (they were published picks); they just
-- no longer count.
create or replace function public.get_streak(p_date text)
returns jsonb language plpgsql stable security definer set search_path = '' as $$
declare
  v_start constant date := date '2026-09-24';
