-- Follow-up to the already-applied atomic prop RPC: categorized scorer picks
-- survive a regular-only exact-game rerun for both football leagues.
...
-- batch. MLB retains its two owned lanes (MLB and MLB HR); a football regular-only
