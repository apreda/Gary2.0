-- A cited-player comparison closes at the earliest game in that set. Explain
-- that decision window while preserving the subject player's game identity,
-- the supplied deadline and all generated advice. Standalone labels stay intact.
...
BEGIN
  ...
  CROSS JOIN LATERAL (SELECT action_label || ': ' || (d->>'headline') AS call_text,
    format(E'Why now: %s\n\nWho it fits: %s\n\nThe risk: %s\n\nWatch next: %s\n\nEvidence as of %s ET.%s',
      ...
      CASE WHEN nullif(d->>'valid_until', '') IS NOT NULL THEN
        CASE WHEN EXISTS (SELECT 1 FROM jsonb_array_elements(d->'evidence') e
          WHERE nullif(e->>'player_id', '') IS NOT NULL AND e->>'player_id' <> d->>'player_id')
          THEN ' Make this comparison before ' || to_char((d->>'valid_until')::timestamptz AT TIME ZONE 'America/New_York', 'Mon DD, HH12:MI AM') || ' ET, when the earliest game in this set starts.'
          ELSE ' Next-game call closes at ' || to_char((d->>'valid_until')::timestamptz AT TIME ZONE 'America/New_York', 'Mon DD, HH12:MI AM') || ' ET.'
        END ELSE '' END) AS read_text) prose;
  RETURN true;
END;
$$;
