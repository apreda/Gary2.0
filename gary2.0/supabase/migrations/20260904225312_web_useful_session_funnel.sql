-- Add a consented session denominator and actual reasoning-view measurement.
-- Historical page-load events retain their original meaning and are not backfilled.
-- Browser/session identifiers are pseudonymous, never account identifiers.
...
create unique index web_events_session_milestone_idx on public.web_events (
  identity, event, (props ->> 'session_id'),
  (case when event = 'meaningful_pick_view' then props ->> 'path' else '' end)
) where event in ('session_started', 'return_visit', 'meaningful_pick_view')
