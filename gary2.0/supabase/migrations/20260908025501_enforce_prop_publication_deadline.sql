-- Production prop publication must remain pregame even after a database-lock
-- wait. No tickets are changed by this migration. Preserve the current atomic
-- identity, original snapshots, owned replacement and football TD behavior.
...
begin
  ...
  -- owns MLB + MLB HR together. An NFL or NCAAF regular-only rerun keeps the prior TD
  ...
  loop
    ...
    if (
      ...
      v_keep_football_td := v_owner_sport in ('NFL', 'NCAAF')
  ...
  -- Check the wall clock AFTER the date lock and merge work. now() is the
  -- transaction start and would wrongly allow a request that waited past kickoff.
  for v_new_pick in select value from jsonb_array_elements(p_new_picks)
  loop
    if coalesce(v_new_pick->>'commence_time', '') !~ 'T.*(Z|[+-][0-9]{2}:?[0-9]{2})$' then
      raise exception 'prop publication requires a timezone-qualified commence_time';
    end if;
    begin
      v_commence_time := (v_new_pick->>'commence_time')::timestamptz;
    exception when invalid_datetime_format or datetime_field_overflow then
      raise exception 'prop publication requires a valid commence_time';
    end;
    if not pg_catalog.isfinite(v_commence_time) or v_commence_time <= pg_catalog.clock_timestamp() then
      raise exception 'prop publication closed: game % has started',
        coalesce(v_new_pick->>'game_id', v_new_pick->>'bdl_game_id', '<unknown>');
    end if;
  end loop;
