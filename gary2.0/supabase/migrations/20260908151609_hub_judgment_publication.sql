-- Add Gary's connected read without rewriting the installed clients' source
-- headline/detail or any sibling research metadata. No new table or RLS policy.
BEGIN;
...
COMMENT ON FUNCTION public.publish_hub_judgment(bigint, date, text, text, text, text, text, jsonb) IS
  'Service-only atomic meta.judgment publication; exact source identity, monotonic evidence time, current ready windows, preserved source content.';
