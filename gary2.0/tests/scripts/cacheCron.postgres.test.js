...
const args = () => ['-h', directory, '-p', '55453', '-U', 'testadmin', '-d', 'postgres', '-X', '-v', 'ON_ERROR_STOP=1', '-At'];
const sql = text => execFileSync(`${bin}/psql`, [...args(), '-c', text], { env: pgEnv, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] }).trim();

const migration = readFileSync(new URL('../../supabase/migrations/20260908034829_secure_cache_refresh_cron.sql', import.meta.url), 'utf8');
...
describe.skipIf(!supported)('cache cron authorization on isolated PostgreSQL', () => {
  beforeAll(() => {
    directory = mkdtempSync('/tmp/gary-cache-cron-pg-');
    try {
  ...
  beforeEach(() => sql(`TRUNCATE cron.job, vault.decrypted_secrets, net.requests;
    INSERT INTO cron.job VALUES
      (4,'live-scores-2min','* * * * *',true,'SELECT old_game_call();'),
      (7,'mlb-field-lineups-30min','*/30 * * * *',false,'SELECT old_prop_call();'),
      (9,'other-job','0 * * * *',true,'SELECT untouched();');
  ...
  it('preserves existing job identity, cadence and active state; reads the service key only at execution', () => {
    ...
    expect(requests).toEqual([
      { url: 'https://xuttubsfgdcjfgmskcol.supabase.co/functions/v1/live-scores', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer fixture-service-v1' }, body: {}, timeout_milliseconds: 30000 },
      { url: 'https://xuttubsfgdcjfgmskcol.supabase.co/functions/v1/mlb-field-lineups', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer fixture-service-v1' }, body: {}, timeout_milliseconds: 60000 },
    ]);
  ...
  it.each(['missing secret', 'empty secret', 'missing job'])('fails atomically for %s before changing any job', scenario => {
    ...
    if (scenario === 'missing job') sql('DELETE FROM cron.job WHERE jobid=7;');
    const before = rows();
