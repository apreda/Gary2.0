...
struct Signal { struct Regression { let day: String }; let slateDate: String?; let reg: Regression? }
struct Board {
  var todayEST: String
  ${block(hub, 'private var tomorrowEST:').replace('private ', '')}
  ${block(hub, 'private func rowSlateDay(').replace('private ', '')}
}
...
for zone in ["UTC", "America/New_York", "America/Los_Angeles", "Asia/Tokyo"] {
  ...
  for key in ["2026-03-08", "2026-11-01", "2026-12-31"] {
    let expected = key == "2026-03-08" ? "2026-03-09" : key == "2026-11-01" ? "2026-11-02" : "2027-01-01"
    let board = Board(todayEST: key)
    precondition(board.tomorrowEST == expected, "Hub tomorrow: \\(zone) \\(key)")
    precondition(board.rowSlateDay(Signal(slateDate: key, reg: .init(day: "tomorrow"))) == expected)
    precondition(board.rowSlateDay(Signal(slateDate: key, reg: .init(day: "tonight"))) == key)
  }
