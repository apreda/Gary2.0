...
func story(_ index: Int, _ kind: String = "connection", _ game: String? = nil) -> Story {
    Story(index: index, kind: kind, gameID: game)
}
${body}
...
describe('native Hub front page selection', () => {
  it.skipIf(!hasSwift)('selects one lead and two supports by game phase and source relevance without a category-only lead override', () => {
    runSwift(`
...
let ranked = [story(40, "platoon", "done"), story(10, "heat", "next"),
              story(80, "park", "playing"), story(22, "bullpen", "next")]
let selected = selector.select(stories: ranked, games: games, now: now)
precondition(selected.lead == 10 && selected.supporting == [22, 80])
...
let repeated = [story(0, "heat", "later"), story(1, "heat", "earlier"),
                story(2, "heat", "later"), story(3, "platoon", "done")]
...
precondition(diverse.lead == 0 && diverse.supporting == [3, 1])
let sameGame = [story(0, "bullpen", "later"), story(1, "heat", "later"), story(2, "streak", "earlier"), story(3, "park", "earlier")]
let spread = selector.select(stories: sameGame, games: games, now: now)
precondition(spread.lead == 0 && spread.supporting == [2, 1], "A different exact matchup surfaces first, then the best unused topic")
let missingIDs = selector.select(stories: [story(0, "bullpen"), story(1, "heat"), story(2, "streak")], games: [], now: now)
precondition(missingIDs.supporting == [1, 2], "Absent game IDs do not create a fictional shared matchup")
...
let selected = selector.select(stories: [story(0, "park", "100"), story(1, "heat", "101"), story(2, "bullpen", "102")], games: games, now: now)
...
let unknowns = selector.select(stories: [story(30, "heat", nil), story(2, "heat", "missing"), story(99, "heat", "")], games: [Game(id: "", startsAt: "2026-09-08T00:00:00Z")], now: now)
