...
const args=()=>['-h',directory,'-p','55459','-U','testadmin','-d','postgres','-X','-v','ON_ERROR_STOP=1','-At'];
...
const payload = {league:'NCAAF',pick_text:'Cubs ML',post_text:'A reason. Cubs ML.',thread_format:'standard',commence_time:etDate+'T18:00:00Z'};
...
describe.skipIf(!supported)('every-game cadence on isolated PostgreSQL', () => {
  beforeAll(() => {
    ...
    execFileSync(`${bin}/pg_ctl`,['-D',`${directory}/data`,'-l',`${directory}/server.log`,'-o',`-k ${directory} -h '' -p 55459`,'-w','start'],{env:pgEnv,stdio:'pipe'}); started=true;
    sql(`create role anon; create role authenticated; create role service_role;
      create table public.social_post_log(id uuid primary key default gen_random_uuid(),post_date text,pick_text text,thread_format text,league text,posted_at timestamptz default now());
    ...
    sql(`create schema cron; create table cron.job(jobid bigint,jobname text,schedule text,active boolean,command text);
      insert into cron.job values(7,'social-auto-post-hourly','*/15 * * * *',true,'authenticated_dispatch');
    ...
    sql(readFileSync(new URL('../../supabase/migrations/20260916210123_social_full_mlb_nfl_coverage.sql', import.meta.url), 'utf8'));
  },30000);
  afterAll(() => {
  ...
  const cap = 12;
  function seed(count) {
    sql(`insert into public.social_publication_intents(post_date,publication_key,log_payload,created_at,updated_at)
      select ${today},'prior-'||n,'${JSON.stringify(payload)}'::jsonb,now()-interval '5 minutes',now()-interval '5 minutes' from generate_series(1,${count})n`);
  }
  it('serializes overlapping runs into one root per four-minute interval', async () => {
    ...
    sql("update public.social_publication_intents set created_at=now()-interval '5 minutes',updated_at=now()-interval '5 minutes'");
    sql(claim('next'));
  ...
  it('MLB and NFL can exceed twelve roots without exhausting the college budget',()=>{
    seed(20);
    sql("update public.social_publication_intents set log_payload=jsonb_set(log_payload,'{league}','\"MLB\"'::jsonb)");
    sql(claim('baseball',{...payload,league:'MLB'}));
    expect(sql('select count(*) from public.social_publication_intents')).toBe('21');
    sql("update public.social_publication_intents set created_at=now()-interval '5 minutes',updated_at=now()-interval '5 minutes'");
    sql(claim('football',{...payload,league:'NFL'}));
    expect(sql('select count(*) from public.social_publication_intents')).toBe('22');
    sql("update public.social_publication_intents set created_at=now()-interval '5 minutes',updated_at=now()-interval '5 minutes'");
    sql(claim('college',payload));
    expect(sql('select count(*) from public.social_publication_intents')).toBe('23');
  });
  it('resuming an old prepared copy reserves the current interval',()=>{
    seed(1); sql(claim('prior-1')); sql(claim('other',{...payload,league:'MLB'}));
    expect(sql('select count(*) from public.social_publication_intents')).toBe('1');
  });
  it('changes only the existing cron schedule',()=>{
    expect(sql("select jobid,jobname,schedule,active,command from cron.job")).toBe('7|social-auto-post-hourly|*/5 * * * *|t|authenticated_dispatch');
  });
