...
const schedule = missing => mocks.schedule.mockImplementation(async date => {
  ...
  return Array.from({ length: day <= 3 ? 2 : 1 }, (_, i) => ({ gamePk: 1000 + day * 2 + i, officialDate: date,
    gameDate: `${date}T${17 + i * 4}:10:00Z`, gameNumber: i + 1,
...
describe('first-inning source copy', () => {
  ...
  it('orders a doubleheader by first pitch, not by game id', async () => {
    schedule(false);
    const read = mocks.schedule.getMockImplementation();
    // Sep 7 has one fixture game; make it a doubleheader whose ids run against first pitch.
    mocks.schedule.mockImplementation(async date => {
      const games = await read(date);
      if (date !== '2026-09-07') return games;
      const [only] = games;
      return [
        { ...only, gamePk: 1015, gameNumber: 1, gameDate: '2026-09-07T17:10:00Z' },
        { ...only, gamePk: 1014, gameNumber: 2, gameDate: '2026-09-07T23:10:00Z',
          linescore: { innings: [{ home: { runs: 1 }, away: { runs: 0 } }] } },
      ];
    });
    const [row] = await computeFirstInning(ctx);
    expect(row.meta.team_sample_games.slice(0, 2)).toEqual([
      { gamePk: 1014, date: '2026-09-07', gameDate: '2026-09-07T23:10:00Z', gameNumber: 2 },
      { gamePk: 1015, date: '2026-09-07', gameDate: '2026-09-07T17:10:00Z', gameNumber: 1 },
    ]);
    expect(row.meta.team_seq.slice(0, 2)).toEqual([1, 0]);
  });
  it('suppresses a team whose same-date games carry no order evidence', async () => {
    schedule(false);
    const read = mocks.schedule.getMockImplementation();
    mocks.schedule.mockImplementation(async date => (await read(date)).map(game => {
      const { gameDate, gameNumber, ...rest } = game;
      return date === '2026-09-03' ? rest : game;
    }));
    expect(await computeFirstInning(ctx)).toEqual([]);
  });
  it('treats a repeated game id with a different first pitch as a conflict', async () => {
    schedule(false);
    const read = mocks.schedule.getMockImplementation();
    const repeated = structuredClone((await read('2026-09-01'))[0]);
    repeated.gameDate = '2026-09-01T03:00:00Z';
    mocks.schedule.mockImplementation(async date => {
      const games = await read(date);
      return date === '2026-09-07' ? [...games, repeated] : games;
    });
