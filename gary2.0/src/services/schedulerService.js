...
export const schedulerService = {
  ...
  shouldGenerateNewPicks: () => {
    ...
    // Special case for April 16, 2025 - generate at 12:22 PM
    const isApril16_2025 = today.getDate() === 16 && today.getMonth() === 3 && today.getFullYear() === 2025;
    
    if (isApril16_2025) {
      targetTime.setHours(12, 22, 0, 0); // 12:22 PM for April 16, 2025
    } else {
    ...
    // Check if it's after target time (12:22 PM on April 16, 2025 or 10:00 AM on other days)
    let isAfterTargetTime;
    if (isApril16_2025) {
      isAfterTargetTime = (currentHour > 12 || (currentHour === 12 && currentMinute >= 22));
    } else {
    ...
    if (isAfterTargetTime && lastGenBeforeTarget) {
      console.log(`Generating new picks at ${isApril16_2025 ? '12:22 PM' : '10:00 AM'} EST`);
      return true;
    }
    ...
    // Check if after target time (12:22 PM on April 16, 2025 or 10:00 AM on other days)
    let isAfterTargetHour;
    if (isApril16_2025) {
      isAfterTargetHour = now.getHours() > 12 || (now.getHours() === 12 && now.getMinutes() >= 22); // 12:22 PM on April 16
    } else {
  ...
  getNextPicksInfo: () => {
    ...
    if (isApril16_2025) {
      todayTargetTime.setHours(12, 22, 0, 0); // 12:22 PM on April 16, 2025
    } else {
