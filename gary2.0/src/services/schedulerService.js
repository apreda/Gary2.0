...
export const schedulerService = {
  ...
  shouldGenerateNewPicks: () => {
    ...
    // Special case for April 16, 2025 - generate at 1:31 PM (test 5)
    const isApril16_2025 = today.getDate() === 16 && today.getMonth() === 3 && today.getFullYear() === 2025;
    
    if (isApril16_2025) {
      targetTime.setHours(13, 31, 0, 0); // 1:31 PM for April 16, 2025 (test 5)
    } else {
    ...
    // Check if it's after target time (1:31 PM on April 16, 2025 or 10:00 AM on other days)
    let isAfterTargetTime;
    if (isApril16_2025) {
      isAfterTargetTime = (currentHour > 13 || (currentHour === 13 && currentMinute >= 31));
    } else {
    ...
    if (isAfterTargetTime && lastGenBeforeTarget) {
      console.log(`Generating new picks at ${isApril16_2025 ? '1:31 PM' : '10:00 AM'} EST`);
      return true;
    }
    ...
    // Check if after target time (1:31 PM on April 16, 2025 or 10:00 AM on other days)
    let isAfterTargetHour;
    if (isApril16_2025) {
      isAfterTargetHour = now.getHours() > 13 || (now.getHours() === 13 && now.getMinutes() >= 31); // 1:31 PM on April 16
    } else {
  ...
  getNextPicksInfo: () => {
    ...
    if (isApril16_2025) {
      todayTargetTime.setHours(13, 31, 0, 0); // 1:31 PM on April 16, 2025
    } else {
