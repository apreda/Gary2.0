  ...
  generateDailyPicks: async function(sport, date = new Date()) {
    try {
      ...
      const qualifiedPicks = picks.filter(pick => {
        ...
        return confidence >= 0.7; // Use 0.7 threshold for normal picks
      });
      
      console.log(`[Enhanced Picks Service] Generated ${qualifiedPicks.length} qualified picks out of ${picks.length} total (confidence >= 0.7)`);
