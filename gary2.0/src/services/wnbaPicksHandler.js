...
export async function generateWNBAPicks(options = {}) {
  ...
  if (typeof options.onlyAtIndex === 'number') {
    ...
    if (idx >= windowed.length) {
      console.log(`[WNBA] Cursor ${idx} exceeds available games (${windowed.length}) - no more games`);
  ...
  for (const game of windowed) {
    ...
    const { exists: alreadyPicked, existingPick } = await gameAlreadyHasPick('WNBA', game.home_team, game.away_team);
    if (alreadyPicked) {
