...
export async function computeFirstInning(ctx) {
  ...
  for (let back = 1; back <= LOOKBACK_DAYS; back++) {
    ...
    try {
      ...
      for (const g of sched) {
        ...
        if (previous && ['date', 'homeId', 'awayId', 'homeR1', 'awayR1'].some(key => previous[key] !== fact[key])) {
  ...
  finals.sort((a, b) => b.date.localeCompare(a.date) || b.gamePk - a.gamePk);
  ...
  for (const f of finals) {
    const source = { gamePk: f.gamePk, date: f.date };
    ...
    if (hAny <= NRFI_MAX && aAny <= NRFI_MAX) {
      ...
      rows.push(makeRow({
        ...
        meta: {
          ...
          home_sample_games: homeSample.map(({ gamePk, date }) => ({ gamePk, date })),
          away_sample_games: awaySample.map(({ gamePk, date }) => ({ gamePk, date })),
          ...enr.meta,
        },
      }));
      continue;
    ...
    if (best) {
      ...
      rows.push(makeRow({
        ...
        meta: {
          ...
          team_sample_games: sampleFor(best.team).map(({ gamePk, date }) => ({ gamePk, date })),
          ...enr.meta,
        },
      }));
    }
