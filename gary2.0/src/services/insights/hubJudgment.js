...
${JSON.stringify(packets.map(({ source_indices, previous, ...packet }) => {
    ...
    return { ...packet,
      eligible_primary_evidence_ids: packet.evidence.filter(entry => entry.primary_eligible === true).map(entry => aliases.get(entry.id)),
      required_evidence_ids: packet.evidence.filter(entry => entry.id === 'current_context').map(entry => aliases.get(entry.id)),
      evidence: packet.evidence.map(entry => ({ ...entry, id: aliases.get(entry.id), primary_eligible: entry.primary_eligible === true })),
