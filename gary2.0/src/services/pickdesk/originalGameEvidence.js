...
export function originalGameEvidence({ result, pick, deskText, first = 'home' }) {
  return {
    ...
    toolResponses: structuredClone(result?._originalToolResponses || []),
    ...(result?._context?.bullpenSnapshot ? { bullpenSnapshot: structuredClone(result._context.bullpenSnapshot) } : {}),
