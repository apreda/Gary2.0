  ...
  async generateMlbPlayerStatsReport(homeTeam, awayTeam, date = new Date().toISOString().split('T')[0]) {
    try {
      ...
      if (pitcherData?.homePitcher) {
        ...
        statsReport += `ERA: ${homePitcher.stats.ERA}, `;
        statsReport += `Record: ${homePitcher.stats.record}, `;
        statsReport += `WHIP: ${homePitcher.stats.WHIP}, `;
        statsReport += `K: ${homePitcher.stats.strikeouts}, `;
        statsReport += `IP: ${homePitcher.stats.inningsPitched}\n`;
      } else {
      ...
      if (pitcherData?.awayPitcher) {
        ...
        statsReport += `ERA: ${awayPitcher.stats.ERA}, `;
        statsReport += `Record: ${awayPitcher.stats.record}, `;
        statsReport += `WHIP: ${awayPitcher.stats.WHIP}, `;
        statsReport += `K: ${awayPitcher.stats.strikeouts}, `;
        statsReport += `IP: ${awayPitcher.stats.inningsPitched}\n`;
      } else {
      ...
      if (homeKeyPlayers.length > 0) {
        ...
        for (const batter of homeBatters.slice(0, 3)) { // Top 3 batters
          ...
          statsReport += `AVG: ${batter.stats.AVG}, `;
          statsReport += `HR: ${batter.stats.HR}, `;
          statsReport += `RBI: ${batter.stats.RBI}\n`;
        }
      } else {
