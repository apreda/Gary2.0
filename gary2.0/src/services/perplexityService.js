...
export const perplexityService = {
	...
	search: async function(query, options = {}) {
		try {
			...
			// Always call server proxy from browser; only server uses direct API with env
			const isBrowser = typeof window !== 'undefined';
			const url = isBrowser ? '/api/perplexity-proxy' : 'https://api.perplexity.ai/chat/completions';
			const headers = isBrowser
				? { 'Content-Type': 'application/json' }
				: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.PERPLEXITY_API_KEY || process.env.VITE_PERPLEXITY_API_KEY || ''}` };

			const response = await axios.post(
				url,
				{ model: requestOptions.model, messages, temperature: requestOptions.temperature, max_tokens: requestOptions.maxTokens },
	...
	/**
	 * The Perplexity API key (server-only). Always empty in the browser.
	 */
	API_KEY: (() => {
		try {
	...
	fetchRealTimeInfo: async function(query, options = {}) {
		try {
			...
			const result = await this.search(optimizedQuery, { temperature: 0.1, maxTokens: 300, ...options });
			if (!result.success) {
	...
	/**
	 * Optimize a query to be more direct and concise
	 */
