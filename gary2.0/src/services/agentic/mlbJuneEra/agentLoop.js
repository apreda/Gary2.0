...
export async function runAgentLoop(systemPrompt, userMessage, sport, homeTeam, awayTeam, options = {}) {
  ...
  const modelOverride = options.modelOverride || process.env.GARY_MODEL_OVERRIDE || null; // ADAPTED (models only): the runner's cascade names the brain per attempt; June read only the env override
