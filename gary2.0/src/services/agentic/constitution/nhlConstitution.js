...
- **Playoff adjustments happen fast.** Between playoff games, coaches shuffle lines, change defensive pairings, alter matchup deployment (last change becomes a weapon), and tweak power-play / penalty-kill units. A line combination that got exposed in one game is often broken up by the next. Do not assume the same matchup problems, line trouble, or special-teams gaps that showed up in one game will repeat — investigate what each coach is likely to adjust.
- **Playoff stats are a separate sample from regular season.** Pace slows, hits/blocks increase, special-teams opportunities tighten, and goalie usage gets concentrated (the #1 starts every game, no rest rotation). When a playoff game is in front of you, weigh playoff-specific stats (this postseason's CF%, xG, goalie SV%, PP/PK%) more heavily than the regular-season baseline. The regular season is context, not the bet.
...
(Each case should be 2-3 paragraphs making the argument for that side as tonight's bet — how it wins this game and what carries it.)`
};
