...
// THE RESEARCH ASSISTANT ON THE SUB (founder, Sep 3 2026, final answer:
// "the bridge does do tools then okay that needs to be the research
// assistant"): Luna through the Codex bridge in tools mode — $0 on the sub,
// a different model from Gary's Sol. If the sub is capped or the bridge
// fails, the Aug 18 Haiku researcher is the backup (metered, ~12¢ a game).
export const GAME_RESEARCH_MODEL = process.env.GARY_RESEARCH_MODEL || 'codex-gpt-5.6-luna';
export const GAME_RESEARCH_FALLBACK_MODEL = process.env.GARY_RESEARCH_FALLBACK_MODEL || 'anthropic-claude-haiku-4-5';
