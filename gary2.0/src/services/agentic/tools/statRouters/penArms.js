...
export async function buildPenArmsForTeam(team, teamName, { todayEt } = {}) {
  ...
  if (uOuts > 0) {
    lines.push(`${teamName} pen as a unit, full season, these ${arms.length} arms: ${((uEr * 27) / uOuts).toFixed(2)} ERA, ${(((uH + uBb) * 3) / uOuts).toFixed(2)} WHIP over ${outsToIp(uOuts)} IP — not a recent-form figure; the last games are under Recent work`);
  }
