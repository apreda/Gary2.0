...
2. Produce 2-4 evidence bullets that cite concrete numbers and note whether they support or challenge the lean.
...
export async function runInvestigatorStage({ gameSummary, hypothesis, tokenPayload }) {
  const cleanTokens = sanitizeTokenRequests(hypothesis.requested_tokens, gameSummary.sport, 4);
  const payload = {
    game: {
  ...
  const normalizedEvidence = Array.isArray(parsed.evidence)
    ? parsed.evidence.slice(0, 4).map((row) => ({
