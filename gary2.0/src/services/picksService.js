...
async function storeDailyPicksInDatabase(picks) {
  ...
  const validPicks = picks.map((pick, index) => {
    ...
    // Filter out picks with confidence below 0.75 threshold for MLB
    ...
    const passesThreshold = confidence >= 0.75;
    if (passesThreshold) {
      console.log(`✅ Including ${sport} pick with confidence ${confidence} (passes 0.75 threshold)`);
    } else {
      console.log(`❌ FILTERING OUT ${sport} pick with confidence ${confidence} (below 0.75 threshold)`);
    }
