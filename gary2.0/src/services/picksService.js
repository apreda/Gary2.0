...
async function storeDailyPicksInDatabase(picks) {
  ...
  const validPicks = picks.map((pick, index) => {
    ...
    // Filter out picks with confidence below 0.80 threshold for MLB
    ...
    // For other sports (mainly MLB), apply the 0.80 confidence threshold
    const passesThreshold = confidence >= 0.80;
    if (passesThreshold) {
      console.log(`✅ Including ${sport} pick with confidence ${confidence} (passes 0.80 threshold)`);
    } else {
      console.log(`❌ FILTERING OUT ${sport} pick with confidence ${confidence} (below 0.80 threshold)`);
    }
    return passesThreshold;
  });

  console.log(`After confidence filtering (>= 0.80 for MLB only, all NBA/NHL picks included), ${validPicks.length} picks remaining from ${picks.length} total`);
