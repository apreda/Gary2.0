...
async function storeDailyPicksInDatabase(picks) {
  ...
  const validPicks = picks.map((pick, index) => {
    ...
    // For other sports (mainly MLB), apply the 0.72 confidence threshold
    return confidence >= 0.72;
  });

  console.log(`After confidence filtering (>= 0.72 for MLB only, all NBA/NHL picks included), ${validPicks.length} picks remaining from ${picks.length} total`);
