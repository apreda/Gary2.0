...
const geminiServiceInstance = {
   ...
   */
  DEFAULT_MODEL: 'gemini-3-flash-preview', // Flash for grounding/props; picks use 3.1 Pro via orchestrator
  
  /**
   * Generate a response from Gemini LLM using secure proxy
      ...
      if (!content || String(content).trim().length === 0) {
        try {
          ...
          if (!fallback || String(fallback).trim().length === 0) {
            throw new Error('Gemini returned empty content after retry');
          }
          content = fallback;
        } catch (e) {
          throw new Error('Gemini returned empty content (no assistant text in response)');
        }
      }

      // Prefer strict top-level JSON parse first; if it fails, attempt to strip common wrappers, else return raw text
      console.log('\n🔍 Gemini response received. Checking for top-level JSON...');
      try {
  ...
  generateGaryAnalysis: async function(gameData, newsData, options = {}) {
    try {
      ...
      console.log('\n🎟️ GENERATING GARY\'S ANALYSIS WITH GEMINI...', {
...

{
  ...
  "league": "NFL" or "NBA" or "MLB" or "NHL" or "NCAAF" or "NCAAB",
...
`
       ...
       */
      // Prepare all game stats in a flexible way - we'll pass whatever we have to Gemini
      // This follows user's direction to be flexible with stats formatting
      let statsSection = '';  
        
      // Add any stats we have - don't be picky about structure, Gemini can parse them
      ...
      if (gameData?.sport === 'MLB' || gameData?.league === 'MLB' || gameData?.sport === 'baseball_mlb') {
        statsSection += `**NOTE: All MLB data is from the current ${new Date().getFullYear()} season**\n\n`;
      }
...
Provide your betting analysis in the exact JSON format specified. Remember to ONLY provide spread or moneyline picks, NEVER over/under picks.`
  ...
  /**
   * Generate prop picks recommendations from Gemini
  ...
  generatePropPicks: async function(prompt, options = {}) {
    try {
      console.log('Generating prop picks from Gemini...');
