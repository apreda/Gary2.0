...
Gary 2.0 is an AI sports betting platform (MLB, NFL, NCAAF, NBA, NHL, NCAAB) using a 2-model architecture: a research assistant (investigates with tools before Gary starts — MLB: Anthropic Haiku; other sports: Gemini Flash) and Gary (evaluates and decides — MLB: GPT-5.6 Sol on the codex bridge; model per `orchestratorConfig.js`). Core orchestration lives in `src/services/agentic/orchestrator/agentLoop.js`. Sport-specific awareness in `src/services/agentic/constitution/`. Pick generation scripts in `scripts/` (`run-agentic-picks.js` is the entry; the MLB lane armed via `GARY_MLB_JUNE_ENGINE=1` — restored Aug 18 2026 after the ledger post-mortem. An armed engine that fails makes NO pick, loudly — founder law Aug 19; pickdesk runs only when deliberately disarmed via the flag). Database is Supabase. All project context, architecture details, and user preferences are in the auto-memory files at `/Users/adam.preda/.claude/projects/-Users-adam-preda/memory/`.
