> **Game model order and Plus login — September 16:** Adam authorized Fable 5.1/xhigh → Astra 6/xhigh (separate Plus, then main Pro) → Opus 5/max for all game picks. Read [GAME_BRAIN_ROUTING](../HANDOFF_2026-09-16_GAME_BRAIN_ROUTING.md). Fresh Plus login and all four provider/account checks passed. Props, research, content and the frozen MLB decision conversation keep their separate policies. Older Sol/NCAAF model instructions are superseded. Verify the installed scheduler environment after activation; source defaults alone are insufficient.
