...
Gary 2.0 is an AI sports betting platform (MLB, NFL, NCAAF, NBA, NHL, NCAAB) using a 2-model architecture: a research assistant (investigates with tools before Gary starts — MLB: Anthropic Haiku; other sports: Gemini Flash) and Gary (evaluates and decides — MLB: GPT-5.6 Sol on the codex bridge; model per `orchestratorConfig.js`). Core orchestration lives in `src/services/agentic/orchestrator/agentLoop.js`. Sport-specific awareness in `src/services/agentic/constitution/`. Pick generation scripts in `scripts/` (`run-agentic-picks.js` is the entry; MLB runs the June engine unconditionally — founder, Aug 27 2026: ONE pick system, failures cascade to fallback MODELS on the same engine; the separate pickdesk game brain is deleted). Database is Supabase. All project context, architecture details, and user preferences are in the auto-memory files at `/Users/adam.preda/.claude/projects/-Users-adam-preda/memory/`.
