...
const isThursdayEST = () => isDayOfWeekEST(4);
...
async function main() {
  ...
  try {
    const props = await propOddsService.getPlayerPropOdds(SPORT_KEY, tnfGame.home_team, tnfGame.away_team, tnfGame.commence_time);
  ...
  if (shouldStore) {
    ...
    // Merge with existing picks (remove old TNF picks first)
    const stored = await mergeAndStorePicks(supabase, dateParam, tnfPicks, p => !p.tnf_pick);
    if (stored) {
