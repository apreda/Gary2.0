...
async function fetchActiveTokens() {
  ...
  // Read with the service role: push_tokens SELECT is being locked down to the
  // service role only (anon registers via the register_push_token RPC). Runs
  // under launchd, which loads .env (has SUPABASE_SERVICE_ROLE_KEY) via loadEnv.
