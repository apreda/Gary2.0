...
export function recordMlbDataFailure(game, error, { incidentDirectory = directory, now = new Date() } = {}) {
  ...
  const file = join(incidentDirectory, `${date}__MLB__${id}.json`);
  let previous;
  try { previous = JSON.parse(readFileSync(file, 'utf8')); } catch { /* first failure */ }
  const incident = { game_id: id, league: 'MLB', home_team: game.home_team || game.homeTeam,
    away_team: game.away_team || game.awayTeam, commence_time: game.commence_time,
    code: 'required_data_unavailable', error: error.error || error.message,
  ...
  console.error(`[MLB Required Data] Publication blocked; incident: ${file}`);
  return incident;
}
