...
export async function runMorningWatch({ now = new Date(), markerPath, cwd, signal, timeoutMs = 75_000, retryMs = 5 * 60_000, runStage = runContentStage }) {
  ...
  if (previous?.date === date && previous.completed_at && previous.status === 'ok' && previous.exit_code === 0) return { skipped: 'already healthy', previous };
  if (previous?.date === date && previous.completed_at && Date.parse(now) - Date.parse(previous.attempted_at) < retryMs) return { skipped: 'failed check cooling down', previous };
