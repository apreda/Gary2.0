// College continuation lanes and NFL practice reports can complete per game.
// A partial pass replaces only rebuilt games, never the whole day's lane.
export function insightRefreshOldIds({ league, category, existing = [], fresh = [] }) {
  const incremental = (String(league).toUpperCase() === 'NCAAF'
    ...
    || (String(league).toUpperCase() === 'NFL' && category === 'practice_report');
