 ...
 * Special feature: Gary picks 1 standard TD + 1 longshot TD for MNF
 */
 ...
 */
async function runMNFTDAnalysis(allTDProps, playerStats, matchup) {
...
## YOUR TASK
This is the primetime MNF game - make it count! You must make TWO TD scorer picks:
...
async function main() {
  ...
  console.log(`\n📊 Total MNF TD props collected: ${allTDProps.length}`);
  ...
  const result = await runMNFTDAnalysis(allTDProps, playerStats, matchup);
  ...
  if (shouldStore) {
    ...
    if (insertError) {
      ...
      console.log(`✅ Stored ${mnfPicks.length} MNF TD picks (1 standard + 1 longshot)`);
    }
  }
