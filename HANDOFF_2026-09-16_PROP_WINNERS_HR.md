...
3. Dedicated HR-only runs bypassed the screen and reused the two-core-props request. They now use the same HR screen and HR-only request. Unsupported shortlisted HR markets cannot leak back through the sheets or an unscreened branch; off-menu output fails visibly.
